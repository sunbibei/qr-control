...
{	
  ...
  IMU( double roll=0, double pitch=0, double yaw=0) : roll(roll), pitch(pitch), yaw(yaw){}    
  
  IMU operator = ( const IMU& A )  
  {  
    return IMU(A.roll, A.pitch, A.yaw);  
  }  
};
...
struct _Angle_Leg
{
	...
	double hip;
	double knee;
  _Angle_Leg(float pitch = 0, float hip = 0, float knee = 0)  : pitch(pitch), hip(hip), knee(knee){}
};
